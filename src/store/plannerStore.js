...
const usePlannerStore = create((set) => ({
  ...
  updateRoomDimension: (dimension, value) =>
    set((state) => ({
      roomDimensions: {
  ...
  // Состояния для активного инструмента
  activeToolType: null, // 'wall', 'floor', 'furniture'
  activeToolData: null,

  // Функции для управления активным инструментом
  ...
  // Функции для применения текстур
  setWallTexture: (textureId, wallId) =>
    set((state) => ({
      walls: state.walls.map((wall) =>
        wall.id === wallId ? { ...wall, textureId } : wall
      ),
    })),
